...
class AvgClicksPoolingInitializer(nn.Module):
    ...
    def forward(self, features: Tensor, batched_fg_coords_list: list) -> Tensor:
        """
        Forward method
        ...
        for b_num, fg_coords_per_image in enumerate(batched_fg_coords_list):
            ...
            else:  ## sometimes we have no clicks!
                descriptors.append(torch.empty((1,0,features[0].shape[1]), dtype=torch.float, device=features[0][0].device))
        return descriptors
